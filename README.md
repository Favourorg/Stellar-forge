...
> **Note:** `VITE_FACTORY_CONTRACT_ID` and `VITE_TOKEN_WASM_HASH` are required client-side; the app displays a misconfiguration screen if either is missing, rather than failing silently at runtime. The Pinata credentials are server-side only — a deployment without them will serve the app fine but reject metadata uploads at the proxy.
...
Then set the Pinata credentials as **server-side** variables (repo-root `.env`
for `vercel dev`, or the Vercel project settings for a deployment). These must
never carry a `VITE_` prefix — that would inline them into the client bundle:
...
#### Metadata uploads fail with "Server misconfiguration"

The IPFS proxy can't see its credentials. Check that `PINATA_API_KEY` and
`PINATA_API_SECRET` are set **without** a `VITE_` prefix, in the environment
the `api/` functions run in (Vercel project settings, or the repo-root `.env`
