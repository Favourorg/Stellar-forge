...
describe("validateFileMagicBytes", () => {
  ...
  it("accepts GIF87a and GIF89a declared as image/gif", () => {
    expect(
      validateFileMagicBytes(gifFixture(64, 64, "87a"), "image/gif").valid,
    ).toBe(true);
    expect(
      validateFileMagicBytes(gifFixture(64, 64, "89a"), "image/gif").valid,
    ).toBe(true);
  });
  ...
  it("rejects files too small to identify", () => {
    expect(
      validateFileMagicBytes(Buffer.from([0xff, 0xd8, 0xff]), "image/jpeg")
        .valid,
    ).toBe(false);
  });
  ...
  it("rejects a JPEG with no SOF marker before scan data", () => {
    ...
    expect(
      validateFileMagicBytes(
        Buffer.concat([soi, sos, Buffer.alloc(8)]),
        "image/jpeg",
      ).valid,
    ).toBe(false);
  });
  ...
  describe("decompression-bomb guard", () => {
    it("rejects a PNG with an oversized dimension", () => {
      const result = validateFileMagicBytes(
        pngFixture(MAX_IMAGE_DIMENSION + 1, 10),
        "image/png",
      );
      expect(result.valid).toBe(false);
    ...
    it("rejects a GIF with maxed-out dimensions", () => {
      expect(
        validateFileMagicBytes(gifFixture(65535, 65535), "image/gif").valid,
      ).toBe(false);
    });

    it("rejects a JPEG with oversized dimensions", () => {
      expect(
        validateFileMagicBytes(jpegFixture(30000, 30000), "image/jpeg").valid,
      ).toBe(false);
    });

    it("rejects zero-dimension images", () => {
      expect(validateFileMagicBytes(pngFixture(0, 0), "image/png").valid).toBe(
        false,
      );
    });
  });
