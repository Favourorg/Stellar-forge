...
describe('ipfsToGatewayUrl', () => {
  it('converts CIDv0 ipfs URI to pinata gateway URL', () => {
    expect(ipfsToGatewayUrl('ipfs://QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco')).toBe(
  ...
  it('rejects non-IPFS URIs rather than passing them through', () => {
    expect(ipfsToGatewayUrl('https://example.com/metadata.json')).toBeNull()
    expect(ipfsToGatewayUrl('data:image/png;base64,AAAA')).toBeNull()
    expect(ipfsToGatewayUrl('ipfs://not-a-valid-cid')).toBeNull()
  })
})
