...
export type CSPDirectives = {
   ...
   * the Pinata read-only gateway, and Sentry ingest for error reporting.
   * IPFS uploads are same-origin ('self') via the api/ipfs/* proxy.
   */
...
export const CSP_DIRECTIVES: CSPDirectives = {
  ...
  'connect-src': [
    ...
    // api.pinata.cloud is deliberately absent: uploads go to our own
    // same-origin /api/ipfs/* proxy, so the browser never calls Pinata's API
    // directly. Re-adding it would re-open the path this CSP exists to close.
