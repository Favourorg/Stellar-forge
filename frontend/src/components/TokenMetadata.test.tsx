...
describe('TokenMetadata', () => {
  it('renders an IPFS metadata image through the Pinata gateway', async () => {
    ...
    render(<TokenMetadata metadataUri="ipfs://QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco" name="Forge Token" symbol="FORGE" />)

    const image = await screen.findByRole('img', { name: 'Forge Token' })

    expect(mockedGetMetadata).toHaveBeenCalledWith('ipfs://QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco')
    expect(image).toHaveAttribute('src', 'https://gateway.pinata.cloud/ipfs/QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco')
