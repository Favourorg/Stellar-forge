  ...
  useEffect(() => {
    ...
    ipfsService
      ...
      .then((data: TokenMetadataResponse) => {
        if (cancelled) return
        // A null gateway URL means the metadata's image wasn't a well-formed
        // ipfs:// URI. Metadata is attacker-controlled, so surface it as an
        // error rather than letting an arbitrary URL reach <img src>.
        const imageUrl = data?.image ? ipfsToGatewayUrl(data.image) : null
        if (!imageUrl) {
